...
from trans_llm import TranslatorClass
import tenacity
from merge_subtitle import SubtitleMerger
from merge_video_srt import add_subtitles_and_mix_audio
...
def GPTTranslate(texts, key, model, url):
...
def srtFileGPTTran(model, gpt_url, sourceFileNameAndPath, outputFileNameAndPath, key):
    srtContent = open(sourceFileNameAndPath, "r", encoding="utf-8").read()
    subGenerator = srt.parse(srtContent)
    subTitleList = list(subGenerator)
    contentList = []
    ...
    contentList = GPTTranslate(contentList, key, model, gpt_url)
...
def srtToVoiceEdge(srtFileNameAndPath, outputDir, charactor = "zh-CN-XiaoyiNeural"):
    ...
    for i in range(len(fileMp3Names)):
        ...
        print("mp3FileAndPath:", mp3FileAndPath)
...
if __name__ == "__main__":
        ...
        try:
            ...
            if paramDict["srt merge translate tool"] == "deepl":
                ...
                if paramDict['srt merge translate key'] == '':
                    ...
                    executeLog.write(logStr)
                    sys.exit(-1)
                if paramDict['srt merge translate url'] == '':
                    logStr = "[WORK x] Error: GPT URL is not provided. Please provide it in the parameter file."
                    executeLog.write(logStr)
                    sys.exit(-1)
                srtFileGPTTran(paramDict['srt merge translate tool'], 
                               paramDict['srt merge translate url'], 
    ...
    # 融合中英文字幕：
    assEnFileName = "merged_subtitles.ass"
    assEnFileNameAndPath = os.path.join(workPath, assEnFileName)
    print("assEnFileNameAndPath:", assEnFileNameAndPath)
    merger = SubtitleMerger(chinese_sub_path=srtZhFileNameAndPath, 
                            english_sub_path=srtEnFileNameAndPath,
                            output_path=assEnFileNameAndPath,
                            chinese_font_name="Arial", 
                            chinese_font_size=12,
    ...
    if paramDict["video zh preview"]:
        try:
            ...
            add_subtitles_and_mix_audio(input_video=sourceVideoNameAndPath, 
                                        input_subtitle=assEnFileNameAndPath, 
                                        input_audio1=voiceConnectedNameAndPath, 
                                        input_audio2=insturmentNameAndPath,
                                        output_video=previewVideoNameAndPath)
